...
TARGET_CHANNEL = clean_channel_username(os.getenv("TARGET_CHANNEL", ""))


# AI tahlil va filtr sozlamalari
# 1 dan 10 gacha bo'lgan minimal sifat/qiziqarlilik bali (masalan, 7 dan yuqori bo'lsa joylanadi)
MIN_QUALITY_SCORE = int(os.getenv("MIN_QUALITY_SCORE", "7"))

# Kanal uslubi va maxsus yo'riqnoma (AI caption uchun)
CHANNEL_TOPIC = os.getenv("CHANNEL_TOPIC", "Qiziqarli faktlar, texnologiya, hayotiy va motivatsion kontent")
...
def validate_config() -> list[str]:
    ...
    if not TELEGRAM_API_ID or TELEGRAM_API_ID == 0:
        errors.append("TELEGRAM_API_ID belgilanmagan (.env faylida)")
    if not TELEGRAM_API_HASH:
        errors.append("TELEGRAM_API_HASH belgilanmagan (.env faylida)")
    if not GEMINI_API_KEY:
        errors.append("GEMINI_API_KEY belgilanmagan (.env faylida)")
